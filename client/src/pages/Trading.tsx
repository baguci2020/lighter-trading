...
function KLineChart({ marketId, resolution, onResolutionChange }: {
  ...
  // Resolution in milliseconds map
  const resMs: Record<string, number> = {
    "1m": 60_000, "5m": 300_000, "15m": 900_000,
    "1h": 3_600_000, "4h": 14_400_000, "1d": 86_400_000,
  };

  // Full historical fetch (200 bars)
  ...
  // Incremental update: fetch only the latest 2 bars and update via updateData
  const fetchLatestCandle = useCallback(async (res: string, mid: number) => {
    if (!seriesRef.current || !initialLoadedRef.current) return;
    try {
      const now = Date.now();
      const barMs = resMs[res] || 900_000;
      // Fetch last 3 bars to ensure we get the current in-progress bar
      const start = now - barMs * 3;
      const url = `https://mainnet.zklighter.elliot.ai/api/v1/candles?market_id=${mid}&resolution=${res}&start_timestamp=${start}&end_timestamp=${now}&count_back=3`;
      const resp = await fetch(url);
      const json = await resp.json();
      const rawCandles = (json.c || json.candles || []) as Record<string, number>[];
      if (!rawCandles.length) return;
      // Update each recent candle (handles both current bar update and new bar creation)
  ...
  useEffect(() => {
    ...
    return () => { ro.disconnect(); chart.remove(); chartRef.current = null; seriesRef.current = null; initialLoadedRef.current = false; };
  }, []);

  // Load full history when market or resolution changes
  useEffect(() => {
    initialLoadedRef.current = false;
    if (seriesRef.current) fetchCandles(resolution, marketId);
  }, [marketId, resolution, fetchCandles]);

  // Real-time polling: update the latest candle every 5 seconds
  useEffect(() => {
    const pollInterval = 5000; // 5 seconds
    const timer = setInterval(() => {
      fetchLatestCandle(resolution, marketId);
    }, pollInterval);
    return () => clearInterval(timer);
  }, [marketId, resolution, fetchLatestCandle]);

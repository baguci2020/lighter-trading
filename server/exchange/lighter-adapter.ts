...
export class LighterAdapter implements IExchangeService {
  ...
  // 代理 axios 实例：仅用于下单/撤单等写操作，绕过加拿大地区限制
  private get proxyApi() {
    const proxyUrl = process.env.CLASH_PROXY_URL || "http://127.0.0.1:7890";
    ...
    return axios.create({
      ...
      headers: { accept: "application/json" },
      httpsAgent: agent,
      proxy: false, // 禁用 axios 内置代理检测，使用 httpsAgent
    });
  }
  ...
  async createOrder(params: CreateOrderParams): Promise<Order> {
    ...
    // Submit the signed transaction via proxy (bypass Canada geo-restriction)
    const response = await this.proxyApi.post("/api/v1/transaction", {
      tx_type: txType,
      tx_info: txInfo,
    });
  ...
  async cancelOrder(params: CancelOrderParams): Promise<boolean> {
    ...
    // Submit cancel via proxy (bypass Canada geo-restriction)
    await this.proxyApi.post("/api/v1/transaction", {
      tx_type: txType,
      tx_info: txInfo,
    });
    return true;
